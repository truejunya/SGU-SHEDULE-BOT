...
class vk_api {
    ...
    public function uploadDocsMessages($id, $local_file_path, $title = null) {
        $data = json_decode(file_get_contents('php://input'));
        if (!isset($title)) $title = preg_replace("!.*?/!", '', $local_file_path);
    ...
    public function getUploadServerMessages($peer_id, $selector = 'doc') {
        $result = null;
        $data = json_decode(file_get_contents('php://input'));
        if ($selector == 'doc') $result = $this->request('docs.getMessagesUploadServer', ['type' => 'doc', 'peer_id' => $peer_id]);
        else if ($selector == 'photo') $result = $this->request('photos.getMessagesUploadServer', ['peer_id' => $peer_id]);
        return $result;
    }
    ...
    public function sendButton($sendID, $message, $gl_massiv = [], $one_time = False) {
        ...
        foreach ($gl_massiv as $button_str) {
            ...
            foreach ($button_str as $button) {
                ...
                if ($button[0] != null) $buttons[$i][$j]["action"]["payload"] = json_encode($button[0], JSON_UNESCAPED_UNICODE);
        ...
        $buttons = array("one_time" => $one_time, "buttons" => $buttons);
    ...
    public function sendDocuments($sendID, $selector = 'doc') {
        if ($selector == 'doc') return $this->request('docs.getMessagesUploadServer', array('type' => 'doc', 'peer_id' => $sendID));
        else return $this->request('photos.getMessagesUploadServer', array('peer_id' => $sendID));
    }
    ...
    private function sendFiles($url, $local_file_path, $type = 'file') {
        $post_fields = array($type => new CURLFile(realpath($local_file_path)));
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type:multipart/form-data"));
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
